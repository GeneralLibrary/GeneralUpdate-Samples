...
把 `MainAppName`、`ClientVersion`、`UpdateAppName`、`ProductId` 等身份字段放在 manifest 中（由 Tools 自动生成），把 `UpdateUrl`、`AppSecretKey`、`Token` 等敏感信息写在代码里。身份属于项目结构，不应硬编码；secret 属于安全配置，不应落入版本控制。
:::
### Manifest 字段映射
