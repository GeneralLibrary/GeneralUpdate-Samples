...
- **Client side**: Both Client and Upgrade reference `GeneralUpdate.Core` (since the latest version, the former `GeneralUpdate.ClientCore` and `GeneralUpdate.Common` have been merged into `GeneralUpdate.Core`)
